...
public class MinHeap {
    ...
    // we remove the roo,
    // bring the last element at its place
    // and try to place it at its correct position using siftDown
    public int extract() {
        if (size == 0) {
            throw new IllegalStateException("Heap is empty");
        }
    ...
    // using recursion to implement siftUp function
    // private void siftUp(int idx) {
    //     if(idx == 0) return;
    //     int p = parent(idx);
    //     if(heap[idx] < heap[p]) {
    ...
    // using recursion to implement siftDown
    // private void siftDown(int idx) {
    //     int smallest = idx;
    //     int left = leftChild(idx);
    //     int right = rightChild(idx);
